...
(function () {
  ...
  function syncThemeFlags() {
    const isDark =
      getComputedStyle(html).getPropertyValue("--pv-theme-is-dark").trim() ===
      "1";

    html.classList.toggle("pv-theme-dark", isDark);
  }
...
function promptVault() {
  return {
    ...
    buildFilterQueryText() {
      ...
      if (
        this.filters.maxRating !== undefined &&
    ...
    async loadPrompts() {
      ...
      try {
        ...
        if (
          !(rx && rx.tags) &&
          Array.isArray(this.filters.tags) &&
    ...
    _setRegexField(field, pattern, flags = "i") {
      const base =
        this.filters.regex && typeof this.filters.regex === "object"
          ? { ...this.filters.regex }
          : {};
    ...
    toggleTag(tagName) {
      const tags = Array.isArray(this.filters.tags)
        ? [...this.filters.tags]
        : [];
